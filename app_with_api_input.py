...
st.markdown("""
    ...
    * {
    ...
    section[data-testid="stSidebar"] {
        background: linear-gradient(180deg, #f8f7ff 0%, #e9e4f0 100%);
    }
...
with st.sidebar:
    st.markdown("""
        <div class="main-header" style="margin-bottom: 1.5rem;">
            <div class="main-title" style="font-size: 1.8rem;">⚙️ Settings</div>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("1. Visit console.groq.com")
    st.markdown("2. Sign up/Login")
    st.markdown("3. Go to API Keys section")
    st.markdown("4. Create new key")
    st.markdown("5. Copy and paste here")
    st.markdown("</div>", unsafe_allow_html=True)
...
if submit:
    ...
    elif not all([user_name, user_company, target_company, target_url]):
